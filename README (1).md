...
- Top-right checkboxes: **Only show rows with a PI email / a PI name / an
  institution** — combinable; a row must satisfy every checked box.
- Under each cell's Institution line, a publications link. If NIH links a
  paper to the grant, it reads **Publication Linked To Grant** (click for
  that paper — the most recent one, any year). Otherwise it reads
  **Publications Within The Past 5 Years? Yes/No**, with Yes opening the
  PI's journal articles from OpenAlex (matched by name plus institution
  history; a **?** marks an uncertain match). The popup table has row
  numbers, year, author contribution (first = red, middle = yellow,
  last = green — in biomedicine the last author is usually the lab head),
  and linked titles; every column sorts (contribution cycles green → yellow
  → red first) and column widths drag to resize. The search box filters by university, department,
...
**Keys:** `NCBI_API_KEY` (optional, faster emails) and `OPENALEX_API_KEY`
(required for publications; free tier is $1/day, and the first backfill of
~800 PIs spreads over two runs automatically) — both as repo Actions secrets.
